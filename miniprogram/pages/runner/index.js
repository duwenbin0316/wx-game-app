...
const PL_BODY = [
  [0,0],[0,4],                               // 耳朵（头顶两角凸出）
  [1,0],[1,1],[1,2],[1,3],[1,4],             // 头顶（全宽，不收窄）
  [2,0],[2,1],[2,2],[2,3],[2,4],             // 头部（含眼睛位置）
  [3,0],[3,1],[3,2],[3,3],[3,4],             // 上身
  [4,0],[4,1],[4,2],[4,3],[4,4],             // 下身
];
// 眼睛（深色小方块嵌入头部 row 2）
const PL_EYES = [[2,1],[2,3]];
// 高光（左上区域）
const PL_HL = [[1,0],[1,1],[2,0]];
// 四条腿（两对）：左对 cols 0-1，右对 cols 3-4，中间 col 2 留空
// 动画：左对上/右对下 ↔ 左对下/右对上（交替迈步）
const PL_LEGS_A = [[5,0],[5,1],[6,3],[6,4]];   // 左腿抬，右腿踩
const PL_LEGS_B = [[6,0],[6,1],[5,3],[5,4]];   // 左腿踩，右腿抬
